...
- 2026-05-01: Checked item-details-to-item-details navigation. Suggested item ids (`s1` etc.) were not in `mockProducts`, so details fell back to the first product. Updated details lookup to include both home mock products and suggested products; reset carousel, like state, and scroll when `productId` changes.
